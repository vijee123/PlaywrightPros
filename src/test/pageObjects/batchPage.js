...
export default class batchPage {
    ...
    constructor(page) {
        ...
        // this.nextPage = this.page.locator("//span[@class='p-paginator-icon pi pi-angle-right']");
        // this.lastPage = this.page.locator("//span[@class='p-paginator-icon pi pi-angle-double-right']");
        // this.previousPage = this.page.locator("//span[@class='p-paginator-icon pi pi-angle-left']");
        // this.FirstPage = this.page.locator("//span[@class='p-paginator-icon pi pi-angle-double-left']");
        ...
        this.logout = this.page.locator("//button[@id='logout']");
    }
    ...
    async verifyPaginationEnabled() {
        ...
        console.log("no. of buttons in paginator: ", count);
        for (let i = 0; i < count; i++) {
            const button = buttons.nth(i);
            const isDisabled = await button.isDisabled();

            // Print button index and whether it's disabled
            console.log("button name: ", await button.getAttribute("class"));
    ...
    async fillCreateBatchForm({ batchId, batchDesc, status, noOfClasses }) {
        ...
        const programField = await this.page.locator("//p-dropdownitem").first();
        // if(!prgName){
    ...
    async checkFieldsInPopUp(field, type) {
        ...
        switch (field) {
            case 'programName':
                const programElt = await popUpContent.locator("//input[@placeholder='Select a Program name']");
                const programTag = await programElt.getAttribute('type'); // Call getTagName() on the handle
                console.log("programTag:", programTag);
                return programTag === type;

            case 'batchName':
                const batchNameElt = await popUpContent.locator("//input[@id='batchName']").first();
                const batchNameTag = await batchNameElt.getAttribute('type'); // Call getTagName() on the handle
                console.log("batchNameTag:", batchNameTag);
            ...
            case 'batchNoOfClasses':
                ...
                const batchNoOfClassesTag = await batchNoOfClassesElt.getAttribute('type'); // Get the element handle
                console.log("batchNoOfClassesTag", batchNoOfClassesTag);
            ...
            case 'batchDescription':
                ...
                const batchDescriptionTag = await batchDescriptionElt.getAttribute('type'); // Call getTagName() on the handle
                console.log("batchDescriptionTag", batchDescriptionTag);
                return batchDescriptionTag === type;

            case 'batchStatus':
                const batchStatusElt = await popUpContent.locator("//div[@class='radio ng-star-inserted']");
                const radio = await batchStatusElt.locator("p-radiobutton").count();
                if (radio == 2) { return true; }
                return false;
  ...
  {    
    ...
    async logoutSuccess() {
        ...
        if (currUrl == "https://playwright-frontend-app-a9ea85794ad9.herokuapp.com/login") {
